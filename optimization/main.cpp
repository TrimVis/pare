...
void get_function_stats_from_db(int& no_benchs, int& n, std::vector<int>& uids,
                                std::vector<int>& len_c, std::vector<std::vector<int>>& C,
    ...
    if (rc == SQLITE_ROW) {
        ...
        // I limit the no of benches being analyzed to 1/10th of the actual size
        no_benchs = std::min(no_benchs, 44000);
    } else {
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        int li = std::min(bcount, no_benchs);
        std::vector<int> Ci(no_benchs, 0);
        for (int i = 0; i < li; ++i) {
            Ci[i] = 1;
        }
        C.push_back(Ci);
    ...
    n = uids.size();
    B.resize(no_benchs, std::vector<int>(n, 0));
    for (int i = 0; i < n; ++i) {
...
int main() {
    try {
        ...
        for (int i = 0; i < no_benchs; ++i) {
            ...
            for (int j : J_i) {
                model.addConstr(z[i] <= O[j], "c_bench_" + std::to_string(i) + "_upper_" + std::to_string(j));
            }
            ...
            model.addConstr(z[i] >= sum_Oj - J_i.size() + 1, "c_bench_" + std::to_string(i) + "_lower");
        }
        ...
        if (model.get(GRB_IntAttr_Status) == GRB_OPTIMAL) {
            ...
            double sum_OX = 0.0;
            for (int i = 0; i < n; ++i) {
                sum_OX += O[i].get(GRB_DoubleAttr_X);
            }
            std::cout << "No functions in use: " << sum_OX << std::endl;
            ...
            std::cout << "Could not find optimal result. Exit status: " << statusStr << std::endl;
        }
